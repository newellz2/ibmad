...
mod port_info_tests {
    ...
    fn get_port_info_lid_success() {
        ...
        log::trace!(
            "get_port_info_lid_success - bytes, length: {}, bytes: {:?}",
            dr_smp_bytes.len(),
            dr_smp_bytes
        );
    ...
    fn set_port_info_lid_success() {
        ...
        log::trace!(
            "set_port_info_lid_success - MAD bytes, length: {}, bytes: {:?}",
            dr_smp_bytes.len(),
            dr_smp_bytes
        );
        match ibmad::mad::dr_smp_mad::from_bytes(dr_smp_bytes) {
    ...
    fn send_dr_port_info_success() {
        ...
        match ibmad::ca::get_cas() {
            Ok(cas) => {
                ...
                let hca = &cas[0];
                match ibmad::mad::open_smp_port(hca) {
                    Ok(mut port) => {
                        log::debug!(
                            "tests - send_dr_port_info_success - Opened IB MAD Port: {:?}",
                            port
                        );
                        if let Ok(agent_id) = ibmad::mad::register_agent(&mut port, 0x81) {
                            log::debug!(
                                "tests - send_dr_port_info_success - Registered agent: {}",
                                agent_id
                            );
                            ...
                            log::debug!(
                                "tests - send_dr_port_info_success -  Sending MAD: {:?}",
                                umad
                            );
                            ...
                            match r {
                                Ok(s) => {
                                    log::debug!(
                                        "tests - send_dr_port_info_success -  Sending Successful, size: {:?}",
                                        s
                                    );
                                }
                                Err(e) => {
                                    log::debug!(
                                        "tests - send_dr_port_info_success -  Sending MAD Failed: {:?}",
                                        e
                                    );
                                }
                            }
                            ...
                            log::debug!(
                                "tests - send_dr_port_info_success -  pi_mad.lid: {}",
                                pi_mad.lid()
                            );
                            log::debug!(
                                "tests - send_dr_port_info_success -  pi_mad.master_sm_lid: {}",
                                pi_mad.master_sm_lid()
                            );
                        }
                    }
                    Err(e) => {
